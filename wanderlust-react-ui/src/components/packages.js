...
class Packages extends Component {
    ...
    getPackages = (continent) => {
        axios.get(backendUrlPackage + "/" + continent)
            .then((response) => {
                this.setState({ packages: response.data, show: true })
    ...
    getHotDeals = () => {
        axios.get(backendUrlPackage + '/hotDeals')
            .then((response) => {
                this.setState({ packages: response.data, show: true, errorMessage: null })
            })
            .catch((error) => {
    ...
    loadBookingPage = (dealId) => {
        ...
        sessionStorage.setItem('price', this.state.totalCharges)
        ...
        if (!this.state.errorMessage) {
                ...
                let element = (
                    <div className="card bg-light text-dark package-card" key={mypackage.destinationId}>
                        <div className="card-body row">
                            ...
                            <div className="col-md-5 offset-md-3 vl">
                            ...
                            <div className="col-md-3 ">
                                ...
                                <div className="text-center text-success"><h6>Rs. {mypackage.chargesPerPerson}</h6></div><br /><br />
                                <div><button className="btn btn-lg btn-primary book btn-block" onClick={() => this.getitinerary(mypackage)}>View Details</button></div><br />
                                <div><button className="btn btn-lg btn-primary book btn-block" onClick={() => this.openBooking(mypackage)}>Book </button>  </div>
                            </div>
                        </div>
                    </div>
                )
            ...
            let element = (
                <div className="offset-md-2">
                    <h2>Sorry we don't operate in this Destination.</h2><br /><br />
                    <a href="/packages" className="btn btn-success">Click Here to checkout our Hot Deals</a><br /><br /><br />
                </div>
            )
            return element;
        }
                    ...
                    <TabView activeIndex={Number(this.state.index)} onTabChange={(e) => this.setState({ index: e.index })}>
                        ...
                        <TabPanel header="Book">
                            ...
                            <form onSubmit={this.handleSubmit}>
                                ...
                                <div className="form-group">
                                    <Button id="buttonCalc" className="btn btn-primary" type="submit" disabled={!this.state.bookingFormValid.buttonActive} label="Calculate Charges" ></Button>&nbsp;
                                </div>
                            </form>
                            {!this.state.totalCharges ?
                                (
                                    <React.Fragment><span>**Charges Exclude flight charges.</span><br /><br /></React.Fragment>
                                )
                                :
                                (
                            ...
                            <div className="text-center">
                                <Button disabled={!this.state.bookingFormValid.buttonActive} className="btn btn-success" onClick={() => this.loadBookingPage(this.state.deal.destinationId)} label="Book"></Button>
                                &nbsp; &nbsp; &nbsp;
                                <Button type="button" className="btn btn-primary" onClick={(e) => { this.setState({ showItinerary: false }); this.resetform() }} label="Cancel"></Button>
                            </div>
                        </TabPanel>
                    </TabView>
                </Sidebar>
